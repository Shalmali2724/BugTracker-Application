...
public class EmployeeController extends Exception {
	...
	public ResponseEntity<String> deleteEmployeeById(@PathVariable("employeeId") long employeeId,@RequestBody Employee employee) throws EmployeeNotFoundException
	{
	...
	public ResponseEntity<String> updateBugStatus(@PathVariable("bugId") long bugId,@RequestBody BugUpdateDto bugupdatedto)
	{
	...
	//here we are calling Project details for employee Id
	@GetMapping("/getAllProject/ByEmployeeId/{empId}")
	public List<Project> getAllProject(@PathVariable("empId") long empId) {
		List<Project> proj = employeeService.getAllProjectByEmpId(empId);
